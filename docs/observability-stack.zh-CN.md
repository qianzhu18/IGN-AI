...
3. 创建 UptimeRobot 监控，先监控首页和 `/api/health`。
4. 创建 Sentry 项目，或先启用 PostHog Error Tracking。
5. 重新部署 Vercel，确认 PostHog 能收到 `$pageview` 和业务事件。
6. 一周后复盘首页转化、活动点击、文章点击和加入申请数据。
