...
IGNAI 官网继续使用 **NotionNext + Notion** 作为生产架构。当前业务以活动、回顾、文章、成员与少量站点配置为主，不建设第二套通用 CMS；Notion 是内容后台，NotionNext 是前台与发布层，R2 仅负责稳定媒体。

当前优化重点是保持 Notion 字段与官网展示一一对应，避免用本地静态内容覆盖后台字段。Payload 迁移分支保留为历史实验，不进入当前生产路线。
...
1. 稳定 Notion Event / Record 字段契约和发布体验
2. 保证活动、回顾、首页与详情页读取同一份动态内容
3. 收束 Member MVP 的剩余体验项
4. 准备可复用的数据层改动与 upstreamable PR 叙事
