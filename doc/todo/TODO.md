...
- [x] **UI-04** Hero 区域微调 ✅
  ...
  - [x] 继续增强桌面端首屏品牌识别（品牌名、定位、行动入口层次更清晰）
  - [x] 继续重排移动端首屏内容逻辑，避免桌面信息直接压缩到手机视口
- [x] **UI-05** 成员展示区精调 ✅
  - 散点布局自适应（移动端 50 人 / 桌面 100 人）
  - 容器添加径向渐变背景增加深度感
  - 预览卡片已含完整信息（头像、姓名、角色、简介、quote）
- [x] **UI-06** 活动区块优化 ✅
...
- [x] **UI-07** 成员目录页改版 ✅ 编号手风琴分组
  - [ ] 搜索/筛选功能（后续迭代）
- [x] **UI-08** 成员详情页打磨 ✅
  - 社交链接添加 SVG 图标（GitHub / X / LinkedIn / Website）
  - Featured 成员头像添加渐变光晕背景
  - 返回链接已有增大点击区域（-ml-3 px-3 py-2）
...
- [x] **UI-09** 导航栏移动端体验 ✅
  ...
  - [x] 清理移动端菜单与悬浮控件中的蓝色默认态，统一为 IGNAI 品牌色
  - [x] 复查移动端菜单展开后与首屏内容的衔接关系，确保信息表达连续
