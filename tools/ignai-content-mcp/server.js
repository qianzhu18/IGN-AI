...
const tools = [
  {
    name: 'ignai_status',
    description: 'Check the IGNAI Notion publishing configuration without exposing secrets.',
    inputSchema: {
      type: 'object',
      properties: {},
  ...
  },
  {
    ...
    description: 'Turn IGNAI website content into a Notion page payload preview. Never write ext: use first-class properties, page blocks, and the native Page Cover. This does not write to Notion.',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description: 'Create an IGNAI Notion draft page. Never write ext: use first-class properties, page blocks, and the native Page Cover. The default status is Invisible, not Published.',
    inputSchema: {
      type: 'object',
      required: ['contentType', 'title'],
      properties: {
  ...
  },
  {
    ...
    description: 'Read the schema for an allowed IGNAI Notion data source.',
    inputSchema: {
      type: 'object',
      properties: {
        contentType: {
  ...
  },
  {
    ...
    description: 'Query pages from an allowed IGNAI Notion data source and return safe property values.',
    inputSchema: {
      type: 'object',
      properties: {
        contentType: {
  ...
  },
  {
    ...
    description: 'Refresh the IGNAI frontend cache through the existing ops-protected revalidate API.',
    inputSchema: {
      type: 'object',
      properties: {
        baseUrl: { type: 'string' },
...
const resources = [
  {
    uri: 'ignai://status/config',
    name: 'IGNAI content pipeline status',
    mimeType: 'application/json',
    description: 'Safe configuration status for the Notion writing layer.'
  },
  {
    uri: 'ignai://schemas/content',
    name: 'IGNAI Notion content schema',
    mimeType: 'text/markdown',
    description: 'Content types and fields expected by the website.'
  },
  {
    uri: 'ignai://guides/workflow',
    name: 'IGNAI content publishing workflow',
    mimeType: 'text/markdown',
...
const prompts = [
  {
    name: 'ignai-official-content-brief',
    description: 'Draft public website copy in IGNAI official voice.',
    arguments: [
      {
        name: 'contentType',
...
function getEnvFileCandidates() {
  ...
  const explicitFiles = []
  const envFileFromEnv = process.env.IGNAI_MCP_ENV_FILE || process.env.MCP_ENV_FILE || ''
  if (envFileFromEnv) {
...
function allowUnlistedDataSource() {
  return process.env.IGNAI_MCP_ALLOW_UNLISTED_DATA_SOURCE === 'true'
}
...
function assertDataSourceAllowed(dataSourceId) {
  ...
  throw new Error(
    'Data source is not in the IGNAI MCP allow-list. Configure its env var or set IGNAI_MCP_ALLOW_UNLISTED_DATA_SOURCE=true.'
  )
}
...
async function callTool(name, args = {}) {
  if (name === 'ignai_status') return getStatus()
...
function getStatus() {
  const envStatus = {
    ...
    IGNAI_MCP_ALLOW_UNLISTED_DATA_SOURCE: allowUnlistedDataSource()
  }
...
function getResource(uri) {
  ...
  if (uri === 'ignai://schemas/content') {
    return {
      mimeType: 'text/markdown',
      text: `# IGNAI Notion Content Schema
...
By default, explicit dataSourceId/page operations are restricted to configured IGNAI data source ids. Set IGNAI_MCP_ALLOW_UNLISTED_DATA_SOURCE=true only for a trusted local debugging session.
`
    }
  }

  if (uri === 'ignai://guides/workflow') {
    return {
      mimeType: 'text/markdown',
      text: `# IGNAI Content Publishing Workflow

1. Use ignai_status to verify Notion token, data sources, and ops password.
...
function getPrompt(name, args = {}) {
  if (name === 'ignai-official-content-brief') {
    return {
      description: 'Draft IGNAI website copy.',
      messages: [
        {
          role: 'user',
          content: {
            type: 'text',
            text: `Write ${args.contentType || 'website'} content for IGNAI.

Goal: ${args.goal || 'make the community proof clearer'}

Voice:
- public, concrete, warm, action-oriented
- prove real community work through members, events, records, projects, and open-source traces
      ...
      messages: [
        {
          ...
          content: {
            type: 'text',
            text: `Convert the approved IGNAI ${args.contentType || 'content'} copy into arguments for build_notion_draft.
  ...
  switch (method) {
    case 'initialize':
      return {
        ...
        serverInfo: {
          name: 'ignai-content-mcp',
          version: '0.2.0'
        },
        instructions: 'Use this server to draft IGNAI website content with the NotionNext-native write contract: never write ext; use first-class properties, page blocks, and the native Page Cover. Drafts default to Invisible.'
      }
    case 'ping':
      return {}
    case 'tools/list':
