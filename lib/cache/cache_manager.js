...
const DEV_CACHE_TTL = 10 * 60 // 10 minutes in dev — run `yarn cache:clear` for fresh data
const PROD_CACHE_TTL = 60 // 60 seconds in prod — aligns with the shortest ISR revalidate so Notion edits show up within ~1-2 minutes instead of waiting for the next deploy
...
export async function setDataToCache(key, data, customCacheTime) {
  ...
  const ttl = customCacheTime ?? (!BLOG.isProd ? DEV_CACHE_TTL : PROD_CACHE_TTL)
