...
const EVENT_STATUSES = new Set(['open', 'planning', 'closed', 'finished'])
...
export function normalizeNotionEvent(event) {
  const ext = event?.ext || {}
  const slug = readFirstString(event?.slug, event?.id)
  if (!slug) return null
  ...
  return {
    ...
    status: normalizeEventStatus(ext.status || event?.eventStatus),
    ...
    hosts: readFirstArray(ext.hosts, ext.organizers, event?.hosts),
    notes: readFirstArray(ext.notes, event?.notes),
