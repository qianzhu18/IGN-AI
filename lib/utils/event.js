...
const MOCK_EVENT_SLUGS = new Set([
  '55-opc',
  'ignai-community-kickoff',
  'changsha-ai-builder-night'
])
const EVENT_COVER_FALLBACKS = {
  '2050-community-meetup-2026': '/images/activity-records/2050-youth-gathering.webp',
  'datawhale-campus-promotion-2026': '/images/activity-records/datawhale-campus-promo.webp',
  'guanchai-changli-ai-garden-2026': '/images/activity-records/guanchai-changli-ai-garden.webp',
  'guanchai-ai-product-manager-camp-2026': '/images/activity-records/guanchai-ai-pm-camp.webp',
  'lev0-minicamp-hackathon-2026': '/images/activity-records/lev0-minicamp-hackathon-award.webp',
  'malangshan-ai-agent-forum-2026': '/images/community-evidence/05-malangshan-ai-agent-forum.webp',
  'sanrenxing-ai-changsha-2026': '/images/activity-records/sanrenxing-ai-changsha-2026.webp',
  'zhijisong-ai-skillathon-2026': '/images/activity-records/zhijisong-minicamp-changsha-2026.webp',
  'zhijisong-minicamp-changsha-2026': '/images/activity-records/zhijisong-minicamp-changsha-2026.webp'
}
        ...
        ? item.media
          .map(asset => {
            ...
            return {
              ...
              orientation: readFirstString(asset.orientation) === 'portrait' ? 'portrait' : undefined
            }
          })
          .filter(Boolean)
        : []
...
export function getEventCoverFallback(event) {
  ...
  return EVENT_COVER_FALLBACKS[slug] || '/images/activity-records/geekathon-community-launch.webp'
}
...
export function normalizeNotionEvent(event) {
  ...
  return {
    ...
    status: normalizeEventStatusWithAliases(ext.status || event?.eventStatus || event?.status),
...
export function normalizeEventList(notionEvents = [], fallbackEvents = staticEvents) {
  const normalized = Array.isArray(notionEvents)
    ...
    : []
  const localEvents = Array.isArray(fallbackEvents) ? fallbackEvents.filter(Boolean) : []
  const eventsBySlug = new Map()

  // Local entries cover evidence-backed active work that has not yet been mirrored
  // into Notion. A matching Notion row owns the final public representation.
  localEvents.forEach(event => {
    const slug = normalizeEventSlugValue(event?.slug, event?.title, event?.id)
    if (slug) eventsBySlug.set(slug, event)
  })
...
export function isMockEvent(event) {
  return MOCK_EVENT_SLUGS.has(readFirstString(event?.slug))
}

export function isPublicUpcomingEvent(event, now = new Date()) {
  if (!event || isMockEvent(event)) return false
  ...
  return event.publicListing === true && ['planning', 'ongoing'].includes(event.status)
}
