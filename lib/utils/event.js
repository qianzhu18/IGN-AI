...
export function normalizeNotionEvent(event) {
  ...
  // Prefer top-level category (Chinese labels), map to English kind.
  // Fall back to ext.kind for backward compat.
  ...
  // Prefer top-level date property (event_start/event_end), fall back to ext.
  const eventStart = event?.event_start?.start
  const eventEnd = event?.event_end?.start
  const dateFromProperty = eventStart
    ? eventEnd
      ? `${eventStart} – ${eventEnd}`
      : eventStart
    : null
  ...
  return {
    ...
    location: readFirstString(event?.location, ext.location, '待定'),
