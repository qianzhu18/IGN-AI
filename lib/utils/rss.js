...
const createFeedContent = async post => {
  ...
  if (blockMap) {
    ...
    postForRender.blockMap = adapterNotionBlockMap(postForRender.blockMap)
    // 格式化内容，部分的样式字段格式在此处理
    if (postForRender.blockMap?.block) {
      postForRender.blockMap.block = formatNotionBlock(postForRender.blockMap.block)
    }

    const content = ReactDOMServer.renderToString(<NotionPage post={postForRender} />)
    const regexExp =
