...
function collectAuthorSlugCandidates(post) {
  ...
  return flattenAuthorCandidates(directCandidates).map(normalizeAuthorSlug)
}

function collectAuthorNameCandidates(post) {
  const rawCandidates = [post?.author, post?.authors, post?.ext?.author, post?.ext?.authors]
  return flattenAuthorCandidates(rawCandidates).map(normalizeLookupKey)
}

function flattenAuthorCandidates(values) {
  const result = []
...
export async function processPostData(props, from) {
  ...
  props.post.authors = resolveAuthorsForPost(
    props.post,
    props.allMembers || []
  )
