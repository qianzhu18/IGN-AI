...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  const resolveThemeValue = () => {
    const configTheme = getValue(defaultVal, BLOG[key])

    // IGNAI 是站点级壳，不应被 Notion 内容库里的主题字段反向覆盖。
    if (configTheme === 'ignai') {
      return convertVal(configTheme)
    }

    return convertVal(getValue(extendConfig[key], configTheme))
  }
  ...
  switch (key) {
    ...
    case 'LINK':
      // blog.config.js 中的 LINK 优先于 Notion 数据库中的值
      return convertVal(BLOG[key] || getValue(extendConfig[key], defaultVal))
    case 'AI_SUMMARY_API':
    case 'AI_SUMMARY_KEY':
  ...
  if (['TITLE', 'DESCRIPTION', 'AUTHOR', 'BIO', 'LINK', 'KEYWORDS'].includes(key) && hasVal(BLOG[key])) {
    val = BLOG[key]
  }

  if (!val) {
    // 这里针对部分key做一些兼容处理
    // 若 blog.config.js 已显式设置 TITLE/DESCRIPTION，则优先使用，不从 Notion siteInfo 覆盖
    switch (key) {
      ...
      case 'AVATAR':
        ...
        break
      case 'TITLE':
        val = hasVal(BLOG.TITLE) ? BLOG.TITLE : siteInfo?.title
        break
      case 'DESCRIPTION':
