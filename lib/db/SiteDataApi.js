...
export function getAllMembers({ allPages }) {
  ...
  return [...allPages]
    ...
    .sort((a, b) => {
      const featuredDelta = Number(isTruthyMemberFlag(b?.featured)) - Number(isTruthyMemberFlag(a?.featured))
 ...
 */
async function fetchMembersFromOfficialAPI() {
  ...
  function findPropertyKey(dataSource, logicalName) {
    const configuredName = PROPERTY_NAMES[logicalName]
    if (!configuredName) return null
    // 直接匹配 key（大小写敏感）
    if (dataSource.properties?.[configuredName]) return configuredName
    // 直接匹配 key（小写）
    if (dataSource.properties?.[configuredName.toLowerCase()]) return configuredName.toLowerCase()
    // 按 name 匹配（大小写不敏感）
    const lowerName = configuredName.toLowerCase()
    for (const [key, schema] of Object.entries(dataSource.properties || {})) {
      if (schema?.name?.toLowerCase() === lowerName) return key
    }
    return null
  }
  ...
  try {
    ...
    const dsResponse = await fetch(`${API_BASE}/data_sources/${dataSourceId}`, { headers })
    if (!dsResponse.ok) return []
    const dataSource = await dsResponse.json()

    // 找到各属性的内部 key
    const typeKey = findPropertyKey(dataSource, 'type')
    const statusKey = findPropertyKey(dataSource, 'status')
    if (!typeKey || !statusKey) return []

    // Step 2: 查询 data_source
    const queryResponse = await fetch(`${API_BASE}/data_sources/${dataSourceId}/query`, {
      method: 'POST',
      headers,
    ...
    const members = (queryData.results || []).filter(page => {
      const type = readPropertyValue(page.properties?.[typeKey])
      const status = readPropertyValue(page.properties?.[statusKey])
      return type === 'Member' && status === 'Published'
    })

    // Step 4: 映射属性
    return members.map(page => {
      const props = page.properties
      const getVal = (logicalName) => {
        const key = findPropertyKey(dataSource, logicalName)
      ...
      return {
        id: page.id,
        title,
        slug: slug || `members/${title.toLowerCase().replace(/\s+/g, '-')}`,
