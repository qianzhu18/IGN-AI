...
import { mergeOfficialPages } from './notion/mergeOfficialPages'
...
async function convertNotionToSiteData(
  ...
  if (officialMembers.length > 0) {
    allPages = mergeOfficialPages(allPages, officialMembers)
  }
  ...
  if (officialEvents.length > 0) {
    allPages = mergeOfficialPages(allPages, officialEvents)
  }
  ...
  if (officialRecords.length > 0) {
    allPages = mergeOfficialPages(allPages, officialRecords)
  }
...
export async function fetchMembersFromOfficialAPI() {
  ...
  try {
    ...
    const dsResponse = await notionFetch(`/data_sources/${dataSourceId}`, {
      headers
    })
    ...
    // 找到各属性的内部 key
    const typeKey = findDataSourcePropertyKey(
      dataSource,
      propertyCandidates.type
    )
    ...
    return removeUndefinedValues(
      members.map(page => mapOfficialMemberPage(page, dataSource))
    )
  } catch (err) {
  ...
  try {
    ...
    const eventsWithCover = await Promise.all(
      events.map(async page => {
        try {
          const pageResponse = await notionFetch(`/pages/${page.id}`, {
            headers
          })
          if (pageResponse.ok) {
          ...
          console.warn(
            `[EVENT-SYNC] Failed to fetch page ${page.id}:`,
            err.message
          )
        }
        return page
    ...
    return removeUndefinedValues(
      eventsWithCover.map(page => mapOfficialEventPage(page, dataSource))
    )
  } catch (err) {
...
export async function fetchRecordsFromOfficialAPI() {
  ...
  try {
    ...
    const recordsWithCover = await Promise.all(
      records.map(async page => {
        try {
          ...
          console.warn(
            `[RECORD-SYNC] Failed to fetch page ${page.id}:`,
            err.message
          )
        }
        return page
    ...
    return removeUndefinedValues(
      recordsWithCover.map(page => mapOfficialRecordPage(page, dataSource))
    )
  } catch (err) {
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  ...
  const defaultDescription =
    ...
    '长沙 AI 社区 — 连接创造者，点燃可能性'
  const defaultPageCover =
    NOTION_CONFIG?.HOME_BANNER_IMAGE ||
