...
async function fetchMembersFromOfficialAPI() {
  ...
  try {
    ...
    // Step 4: 映射属性
    return members.map(page => mapOfficialMemberPage(page, dataSource))
  } catch (err) {
 ...
 */
async function fetchEventsFromOfficialAPI() {
  const token = process.env.NOTION_API_TOKEN
  const dataSourceId =
  ...
  try {
    ...
    return events.map(page => mapOfficialEventPage(page, dataSource))
  } catch (err) {
