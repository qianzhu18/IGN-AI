...
export default async function getPageProperties(
  ...
  const pageCoverPosition = Number(value?.format?.page_cover_position)
  if (Number.isFinite(pageCoverPosition)) {
    const clampedPosition = Math.min(1, Math.max(0, pageCoverPosition))
    const yPercent = Number(((1 - clampedPosition) * 100).toFixed(4))
    properties.coverPosition = `center ${yPercent}%`
  }
...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  } else if (properties.type === 'Page' || properties.type === 'Member') {
    properties.href = properties.slug ?? properties.id
